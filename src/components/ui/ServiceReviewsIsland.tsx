...
const ServiceReviewPopup: React.FC<{ onToast: (type: 'success' | 'error', message: string) => void }> = ({ onToast }) => {
  const {
    formData,
    isSubmitting,
    error,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      onToast('success', 'Service review submitted successfully! It will appear after moderation.');
...
const ServiceReviewsIsland: React.FC<ServiceReviewsIslandProps> = ({ 
  ...
  return (
    <>
      ...
      {/* Review Popup */}
      <AnimatePresence>
        {showPopup && <ServiceReviewPopup onToast={addToast} />}
      </AnimatePresence>
